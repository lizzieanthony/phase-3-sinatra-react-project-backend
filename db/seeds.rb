...
Exercise.create(name: "Interval arms", instructions: "holding 5lb weights do 10 counts of butterfly arms, then arm curls, then overhead arm curls, then lateral raise. cycle through each set at least 3 times. increase weights as necessary .", workout_id: w2.id)
Exercise.create(name: "Park Bench Dip", instructions: "balance your palms on the bench and heels on the ground. dip your arms down then up like a backwards pushup for 20 reps. repeat as desired", workout_id: w1.id)
Exercise.create(name: "Park Bench Incline Pushup", instructions: "balance your palms on the bench and toes on the ground. dip your arms down then up in the form of a pushup for 20 reps. repeat as desired", workout_id: w1.id)
